...
contract('SupplyChain', function(accounts) {
    ...
    //1st test
    it("Testing smart contract function addRole() for assigning various user roles ", async () => {
    ...
    it("Testing smart contract function sellItem() that allows a farmer to sell coffee", async() => {
        ...
        // Verify the result set
        assert.equal(resultBufferTwo[4], productPrice, 'Error: Product price is missing or invalid');
        assert.equal(resultBufferTwo[5], itemState, 'Error: Invalid item State');
    ...
    it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async() => {
        ...
        // get farmer and distributor's balance before transaction
        ...
        const result = await supplyChain.buyItem(upc, {from: distributorID, value: productPrice, gasPrice: 0});

        // set the itemState
        const itemState = 4

        // get farmer and distributor's balance after transaction
        const farmerBalanceAfterTransaction = await web3.eth.getBalance(distributorID);
        const distributorBalanceAfterTransaction = await web3.eth.getBalance(originFarmerID);
        ...
        assert.ok(farmerBalanceAfterTransaction > farmerBalanceBeforeTransaction)
        assert.ok(distributorBalanceBeforeTransaction > distributorBalanceAfterTransaction);
    })
